                            ...
                            <tr data-id="{{ $download->id }}">
                                ...
                                <td class="action">
                                    @if ($download->status_id == App\Models\Download\DownloadStatus::ID_COMPLETADO)
                                        <a href="{{ route('admin.downloads.show', $download) }}" class="text-primary">{{ __('Descargar') }}</a>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            form.reset();
            insertRow(json.download_id, json.name, fd.get('url'));
            alert('La descarga ha sido encolada.');
        } catch (err) {
    ...
    function insertRow(id, name, url) {
        const row = table.insertRow(0);
        row.dataset.id = id;
        const currentUser = '{{ Auth::user()->name }}'; // Nombre del usuario autenticado
        const now = new Date().toLocaleString(); // Fecha y hora actual
        row.innerHTML = `
            <td>${id}</td>
            <td>${name}</td>
    ...
    async function poll() {
        try {
            ...
            downloads.forEach(download => {
                ...
                if (download.status_id == {{ App\Models\Download\DownloadStatus::ID_COMPLETADO }}) {
                    row.querySelector('.action').innerHTML =
                        `<a href="{{ url('admin/downloads') }}/${download.id}" class="text-primary">Descargar</a>`;
